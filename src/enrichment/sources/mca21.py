...
class MCA21Source(PublicSource):
    source_id = "mca21"
    ...
    def _search_mca(self, company_name: str) -> Optional[dict]:
        """Search MCA database for company and extract contact info."""
        GLOBAL_RATE_LIMITER.wait(self.domain, self.rate_limit_seconds)
        
        try:
            url = f"https://{self.domain}/cgi-bin/Director_Identification_Number.html"
    ...
    def search(self, record: dict) -> Optional[ContactResult]:
        """Search for shareholder contact using MCA database."""
        ...
        try:
            result = self._search_mca(company_name)
