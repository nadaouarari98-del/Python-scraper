...
class NSESource(PublicSource):
    ...
    def _search_nse(self, company_name: str) -> Optional[dict]:
        """Search NSE for listed company contact information."""
        GLOBAL_RATE_LIMITER.wait(self.domain, self.rate_limit_seconds)
        
        try:
            url = f"https://{self.domain}/companies-listing"
    ...
    def search(self, record: dict) -> Optional[ContactResult]:
        """Search for shareholder contact using NSE database."""
        ...
        try:
            result = self._search_nse(company_name)
