...
class IEPFSource(PublicSource):
    source_id = "iepf"
    domain = "www.iepf.gov.in"
    ...
    def _search_iepf(self, company_name: str) -> Optional[dict]:
        """Search IEPF database for company contact information."""
        GLOBAL_RATE_LIMITER.wait(self.domain, self.rate_limit_seconds)
        
        try:
            url = f"https://{self.domain}/IEPF/Master/companies"
    ...
    def search(self, record: dict) -> Optional[ContactResult]:
        """Search for shareholder contact using IEPF database."""
        ...
        try:
            result = self._search_iepf(company_name)
